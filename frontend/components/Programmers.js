...
const [ findID, setFindID ] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.find(people => people.id === findID).name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: findID ? "gold" : "royalblue", // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setFindID(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
